...
public final class RequestEngine {
	...
	public String getCookie(java.net.URL url) {
		Collection cookies = this.cookieStore.getCookies(url.getHost(), url
				.getPath());
				...
				if (pinfo != null) {
					...
					for (int i = 0; i < parameters.length; i++) {
						...
						if (parameter.isText()) {
							...
							FileInputStream in = new FileInputStream(parameter
									.getFileValue());
							try {
	...
	private String completeGetUrl(String baseURL, ParameterInfo pinfo, String ref)
			throws Exception {
		String newNoRefURL;
		Parameter[] parameters = pinfo.getParameters();
	...
	private void addRequestProperties(URLConnection connection,
		...
		connection.addRequestProperty("X-Java-Version", userAgent
				.getJavaVersion());
	...
	/**
	 * Whether possibly cached request should always be revalidated,
	 * i.e. any expiration information is ignored.
	 */
	...
	/**
	 * Whether the request type should always be obtained from
			...
			try {
				...
				try {
					...
					response = new ClientletResponseImpl(rhandler, connection,
							url, isContentCached, cacheInfo, isCacheable, rhandler.getRequestType());
