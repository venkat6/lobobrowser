...
//    public Collection getBroadRestrictedStores(String hostName) throws IOException {
//    	SecurityManager sm = System.getSecurityManager();
//    	if(sm != null) {
...
//            return Collections.singleton(new RestrictedStore(domainDir, HOST_STORE_QUOTA));    	
//        }
//        else {
//        	Collection restrictedStores = new LinkedList();
//        	File[] domainDirs = hostStoreDir.listFiles(new CookieHostFilenameFilter(hostName));
//        	if(domainDirs != null) {
    ...
    public void run() {
    	for(;;) {
    		try {
    			...
    			synchronized(this) {
    				stores = (RestrictedStore[]) this.restrictedStoreCache.values().toArray(new RestrictedStore[0]);
    			}
